<?php
/**
 * pixelplay functions and definitions
 *
...
function pixelplay_setup() {
	/*
		...
		* If you're building a theme based on pixelplay, use a find and replace
		* to change 'pixelplay' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'pixelplay', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'pixelplay' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'pixelplay_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'pixelplay_setup' );
 ...
 */
function pixelplay_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'pixelplay_content_width', 640 );
}
add_action( 'after_setup_theme', 'pixelplay_content_width', 0 );
 ...
 */
function pixelplay_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'pixelplay' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'pixelplay' ),
...
add_action( 'widgets_init', 'pixelplay_widgets_init' );
...
function enqueue_custom_fonts() {
	if (!is_admin()) {
		wp_register_style('roboto_font', 'https://fonts.googleapis.com/css2?family=Roboto:wght@400;700&display=swap', array(), null, 'all');
